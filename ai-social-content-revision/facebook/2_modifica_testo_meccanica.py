...
"""
Script 2: Modifica meccanica del testo
...
# Input/Output di default
DEFAULT_INPUT_FILE = os.getenv("DEFAULT_INPUT_FILE", "Modifica/posts_senza_match.json")
...
def remove_blocked_hashtags(text: str) -> str:
    """
    Rimuove gli hashtag che contengono sottostringhe bloccate o sono nella lista di ban.
    """
    if not text:
        return text
    ...
    def _replace(match: re.Match) -> str:
        tag = match.group(0)
        lower = tag.lower()
        if lower in HASHTAG_REMOVE_LIST:
            return ""
        return tag
...
def normalize_text_whitespace(s: str) -> str:
    """
    Normalizza lo whitespace del testo per confronti: collassa sequenze di whitespace
    in uno spazio singolo e rimuove spazi iniziali/finali.
    """
    if not s:
        return ""
...
def passthrough_post(post: Dict[str, Any], post_number: int) -> Dict[str, Any]:
    ...
    """
    post_copy = post.copy()
    original_message = post_copy.get("message") or post_copy.get("original_message") or ""
    post_copy["post_number"] = post_number
    post_copy["original_message"] = original_message
    post_copy["status_meccanico"] = "invariato"
    # Evita campi duplicati se non ci sono modifiche
    post_copy.pop("message_meccanico", None)
    post_copy.pop("modified_message", None)
    return post_copy
...
def process_posts(
    ...
    for index, post in enumerate(posts, start=1):
        ...
        original_message = post.get("message") or post.get("original_message") or ""
        ...
        # Confronto normalizzato per ignorare modifiche che sono solo di
        # whitespace/formatting. Vogliamo marcare come "modificato" solo se
        # effettivamente cambia il contenuto (es. rimozione hashtag, sostituzioni).
        if normalize_text_whitespace(modified_message) != normalize_text_whitespace(original_message):
            modified_count += 1
...
def save_results(results: List[Dict[str, Any]], output_path: str) -> None:
    ...
    payload = {
        ...
        "posts": results, # Salvo la chiave 'posts' per coerenza con il file originale
    }
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        help=f"File JSON con i post da elaborare (default: {DEFAULT_INPUT_FILE})",
    )
    parser.add_argument(
        "--output",
